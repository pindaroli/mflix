...
export default class MoviesDAO {
  ...
  static async getMoviesByCountry(countries) {
    ...
    */
    ...
    try {
      ...
      const myCountries=Array.isArray(countries) ? countries : countries.split(", ")
  ...
  static async getMovies({
    ...
    console.log(filters,page,moviesPerPage)
    let queryParams = {}
    if (filters) {
      if ("text" in filters) {
    ...
    try {
      cursor = await movies
        .find(query)
        .project(project)
        .sort(sort)//.skip(page).limit(moviesPerPage)
    } catch (e) {
      console.error(`Unable to issue find command, ${e}`)
  ...
  static async getMovieByID(id) {
    try {
      ...
      */
      ...
      if ((e.errmsg && e.errmsg.includes("E11000 duplicate key error collection"))||
      (e+''.includes('Argument passed in must be a single String of 12 bytes or a string of 24 hex characters')))
        return null
      throw e
    }
  }
